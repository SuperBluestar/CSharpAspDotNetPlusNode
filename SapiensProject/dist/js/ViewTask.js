function CargarBlog(data) {
    ...
    while (counter < arrayLength) {
        ...
        if (m[counter].array != undefined) {
            ...
            _descComent = ' ';
        if (m[counter].idTipoTask == 1) {
            var TipoNotaDiv = '<span style="font-size: .85rem;" class="description"> <b>Evaluación:</b> ' + m[counter].TipoNota + '</span>';
            var icono = ReturnIcono(1);
            if (m[counter].EsConFechaEntrega == 1) {
                var FechaEntrega = '<span style="font-size: .85rem;" class="description"><p> Fecha límite: ' + moment(m[counter].Fecha_Entrega).format("D MMM h:mm a") + _descComent+'</p></span>';
            }
            else if (m[counter].EsConFechaEntrega == 0) {
                var FechaEntrega = '<span style="font-size: .85rem;" class="description"><p> Fecha límite: Sin fecha límite ' + _descComent+'</p></span>';
            }
        }
        else if (m[counter].idTipoTask == 3) {
            var icono = ReturnIcono(3);
            var FechaEntrega = '<span class="description" style="font-size: .85rem;"><p> Fecha en Agenda: ' + moment(m[counter].Fecha_Agenda).format('MMM D') + _descComent+'</p></span>';
            var TipoNotaDiv = '<span class="description" style="font-size: .85rem;"> Material de Clases</span>'
        }
        else if (m[counter].idTipoTask == 2) {
            var icono = ReturnIcono(2);
            var FechaEntrega = '<span class="description" style="font-size: .85rem;"><p> Fecha de Inicio: ' + moment(m[counter].Fecha_Agenda).format("D MMM h:mm a") + _descComent + '</p></span>';
            var TipoNotaDiv = '<span class="description" style="font-size: .85rem;"> Examen en Linea</span>'
        }
        var right = '<button type="button" class="btn btn-box-tool" data-toggle="dropdown" aria-expanded="false"> ' +
            ...
            '</ul>';
        $('#table1').append('<a ' + href + ' class="' + counter + ' list-group-item actions-trigger" style="padding-left: 11px;padding-right:0;padding-bottom: 0px;padding-top:8px;line-height: 1.4;">' +
            '<div style="float: right;position: relative;" id="rig_' + counter + '">' +
            '</div> ' +
            '<div class="user-block">' +
            '' +
            icono +
            '<span style="display: block;width: 150px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;letter-spacing: .01785714em;font-size: 1rem;font-size: .885rem;font-weight: 500;line-height: 1.25rem;color:#3c4043;" class="hidden-sm-up">' + m[counter].Tema + '</span>'+
            '<span class="hidden-xs-down" style="letter-spacing: .01785714em;font-size: 1rem;font-size: .885rem;font-weight: 500;line-height: 1.25rem;color:#3c4043;">' + m[counter].Tema + ' </span>' +
            TipoNotaDiv +
            FechaEntrega +
            '</div>' +
            '</a>');    
